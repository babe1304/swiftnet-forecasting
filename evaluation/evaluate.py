...
class ConfusionMatrix():
    ...
    def update(self, preds, labels):
        ...
        curr_conf_matrix = curr_conf_matrix.reshape((self.num_classes, self.num_classes)).astype(np.int64)
    ...
    def get_metrics(self, verbose=True):
        ...
        for id in range(self.num_classes):
            ...
            class_iou[id] = iou
            if verbose:
                print('\t{} IoU accuracy = {:.2f} %'.format(self.class_info[id] if self.class_info is not None else id,
                                                            iou * 100))
...
def evaluate_semseg(model, data_loader, class_info, observers=(), split=False):
    model.eval()
    ...
    with contextlib.ExitStack() as stack:
        ...
        for step, batch in tqdm(enumerate(data_loader), total=len(data_loader)):
            ...
            if split:
                logits, additional = model.forward_split(batch, image_size=batch['original_labels'].shape[1:3])
            else:
            ...
            # time.sleep(2)
        print('')
        pixel_acc, iou_acc, recall, precision, _, per_class_iou = compute_errors(conf_matrix.get_matrix(), class_info,
                                                                                 verbose=True)
        # print('Mine:')
