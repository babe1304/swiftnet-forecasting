...
custom_desc = '_64x32_shared_256'#'_128x64_first_3x3_rest_5x5'
# current_dir = 'F2F-' + str(f2f_levels) + custom_desc + '_t' + str(timestep) + '_' + begin_time
current_dir = 'F2MF-' + str(f2f_levels) + custom_desc + '_t' + str(timestep) + '_' + begin_time
...
# f2f_model = DeformF2F_N(N=f2f_levels, in_channels=512, out_channels=128, mean=None, std=None)
f2f_model = F2MF(in_channels=512, out_channels=128, mean=None, std=None)
    ...
    with torch.no_grad():
        with tqdm(enumerate(loader), total=len(loader), desc='Testing') as progress:
            for batch_idx, batch in progress:
                ...
                pred_feats, additional = f2f_model(past_feats)
                # pred_feats = f2f_model.unnormalize(pred_feats)

                loss = f2f_model.loss(additional, target_feats)
